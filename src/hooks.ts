...
export default function useThunkReducer<R extends Reducer<any, any>, I>(
  ...
  const thunkDispatch: ThunkDispatch<ReducerState<R>, ReducerAction<R>> = (action) => {
    if (typeof action === 'function') {
      return (action as AsyncAction<ReducerState<R>, ReducerAction<R>>)(dispatch, globalState);
    }
    dispatch(action);
  };
